...
class Final extends React.Component {
  ...
  handleSelection = data => {
    this.setState((prevState) => {
      ...
      const selectedNameUniq = [...new Set([...prevState.selectedName, ...[data.name_fr]])] // there
  ...
  selectAll = datas => {
    ...
    datas.forEach(elem => {
      ...
      allElemsName.push(elem.name_fr) // there
    })
  ...
  render(){
    ...
    if (inputValue.length > 0) {
      filteredData = this.props.data.filter(letter => {
        ...
        const formatedDataTitle = letter.name_fr.toLowerCase().trim() // there
                  ...
                  {filteredData.map(items => (
                      <ItemsContainer onClick={() => this.handleSelection(items)} key={items.id}>
                        <Cards
                          ...
                          description={items.subtitle_fr}
                          image={items.main_image.versions.small.url}
                          goal={items.goal}
                          reached={items.amount_raised}
                          nb_product_sold={items.nb_products_sold}
                          currency={items.currency_display}
                          type={items.type}
                        />
                      </ItemsContainer>
                    )
